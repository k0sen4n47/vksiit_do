...
<div class="dashboard__create-content">
    ...
    <form action="{{ route('admin.subjects.update', $subject->id) }}" method="POST" id="subjectForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="form-create">
            <div class="form-create__top">
                ...
                <div class="form-group">
                    ...
                    @if($subject->image)
                        <div class="current-image mb-2">
                            <img src="{{ asset('storage/' . $subject->image) }}" alt="{{ $subject->name }}" style="max-width: 200px;">
                        </div>
                    @endif
                ...
                <div id="connections-container">
                    @php
                        $connections = $subject->teacherGroupConnections->groupBy('user_id');
                        $index = 0;
                    @endphp

                    @foreach($connections as $teacherId => $teacherConnections)
                        <div class="form-create__connection" data-index="{{ $index }}">
                            <div class="form-create__connection-header">
                                <h2>Связь #{{ $index + 1 }}</h2>
                                <button type="button" class="btn btn-danger btn-sm form-create__connection-remove" {{ $index === 0 ? 'style=display:none;' : '' }}>Удалить</button>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="connections[{{ $index }}][teacher_id]" id="connections[{{ $index }}][teacher_id]" class="form-control teacher-select">
                                    ...
                                    <option value="{{ $teacher->id }}" {{ old("connections.{$index}.teacher_id", $teacherId) == $teacher->id ? 'selected' : '' }}>
                                        {{ $teacher->fio }}
                                    </option>
                                    @endforeach
                                </select>
                                @error("connections.{$index}.teacher_id")
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="connections[{{ $index }}][group_ids][]" id="connections[{{ $index }}][group_ids]" class="form-control group-select" multiple>
                                    ...
                                    <option value="{{ $group->id }}" {{ in_array($group->id, old("connections.{$index}.group_ids", $teacherConnections->pluck('group_id')->toArray())) ? 'selected' : '' }}>
                                        {{ $group->short_name }}-{{ $group->course }}{{ $group->year }}@if(!empty($group->suffix)) {{ $group->suffix }}@endif
                                    </option>
                                    @endforeach
                                </select>
                                @error("connections.{$index}.group_ids")
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
            ...
            <button type="submit" class="btn btn-primary">Обновить предмет</button>
        </div>
    </form>
</div>
