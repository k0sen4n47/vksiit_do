...
<div class="dashboard__create-content">
    ...
    <div class="dashboard__filter">
        <form action="{{ route('admin.subjects.index') }}" method="GET" class="form-filter">
            ...
            <div class="form-filter__buttons">
                ...
                <a href="{{ route('admin.subjects.index') }}" class="filter-unset">
    ...
    @if ($subjects->isEmpty())
    <p>Предметы пока не созданы или не найдены по вашему фильтру.</p>
    @else
    <div class="subject-list admin-list">
        @foreach ($subjects as $subject)
        <div class="subject-list__item admin-list__item">
            <div class="subject-list__info admin-list__info">
                <div class="subject-list__name admin-list__name">Название: {{ $subject->name }}</div>
                <div class="subject-list__abbreviation">Аббревиатура: {{ $subject->abbreviation }}</div>
                @if($subject->image)
                <div class="subject-list__image">
                    <img src="{{ asset('storage/' . $subject->image) }}" alt="{{ $subject->name }}" width="50">
                </div>
                @endif
            </div>
            <div class="subject-list__actions admin-list__actions">
                <a href="{{ route('admin.subjects.edit', $subject) }}" class="btn editor">Редактировать</a>
                <form action="{{ route('admin.subjects.destroy', $subject) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn deletor" onclick="return confirm('Вы уверены, что хотите удалить этот предмет?');">Удалить</button>
                </form>
            </div>
        </div>
        @endforeach
